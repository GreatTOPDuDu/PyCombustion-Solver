...
= -\frac{1}{\rho}\nabla p
  + \nabla \cdot (\nu \nabla \boldsymbol{u})
  + \boldsymbol{g}_{\mathrm{buoy}}
$$

Here $\rho(T, Y_k)$ is the mixture density, $\nu$ is the kinematic viscosity, and $\boldsymbol{g}_{\mathrm{buoy}}$ represents buoyancy forces.
...
Time integration employs first-order explicit (forward Euler) updates for all variables. The global time step $\Delta t$ is restricted by separate convective and diffusive CFL conditions,
...
\qquad
\Delta t = \min(\Delta t_\text{adv}, \Delta t_\text{diff}).
$$

Here $C_\text{adv}$ and $C_\text{diff}$ are user-configurable CFL numbers and $\kappa$ represents the largest relevant diffusion coefficient (thermal, viscous, or species).
...
$$
\boldsymbol{u}^{n+1}
= \boldsymbol{u}^* - \Delta t \; \frac{1}{\rho} \nabla p'.
$$
